...
if( isset($_POST['key']) && isset($_POST['projectId']) && isset($_POST['blocks']) ){                                   //checks if the tag post is there and if its been a proper form post
  ...
  if($_POST['key']==$SQLKEY){
    ...
    if($conn->connect_error){                                                           //checks connection
      ...
      $result=$conn->query('UPDATE `projects` SET `blocks`="' . urlencode($_POST['blocks']) . '" WHERE id=' . $_POST['projectId']);                                                     //runs the posted query
      if($result === false){
