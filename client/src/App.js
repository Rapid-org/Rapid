...
import {
    ...
    RadioGroup,
    Alert,
    Grid, CardContent, Card, Typography, CardActionArea, CardActions
} from '@mui/material';
...
class App extends React.Component {
    ...
    constructor(props) {
        ...
        setInterval(async () => {
            if (!this.state.currentProject && this.projectManager) {
                this.loadProjects();
            }
        ...
        this.projectManager.fetchProjects((status) => {
            if (status === 200) {
                if (this.projectManager.getProjects()) {
                    ...
                    this.showSnackbar("Your session has expired. Please reload the webpage and try again.");
                } else {
    ...
    doExportSelectedProjects(opt_force, opt_zip) {
        ...
        if (this.state.checkedProjects.length > 1) {
            ...
            let project = this.state.checkedProjects[0];
            this.blocklyWorkspace = new BlocklyWorkspace(project, this.projectManager);
            this.blocklyWorkspace.createProjectFile(project, (content, project) => {
                saveAs(content, `${project['name']}.rbx`);
            });
            ...
            !this.state.isLoading ? (<ThemeProvider theme={this.theme}>
                <div className="App">
                    ...
                    >
                        ...
                        <DialogContent dividers id={"classes-data-grid"}>
                            <TextField id="filled-basic" label="Search" onChange={(e) => {
                                let data = this.state;
                                data.classesDataFiler = e.target.value;
                                this.setState(data);
                            }} fullWidth
                                       sx={{marginBottom: '20px', display: this.state.classesData ? 'display' : 'none'}}
                        ...
                        <DialogActions dividers>
                            <Button onClick={() => this.handleCloseImportClassDialog()}>Close</Button>
                            <Button disabled={this.lastFetchedClassesNum <= 50}
                                    onClick={() => this.loadClassesData(true)}>Back</Button>
                            <Button onClick={() => this.loadClassesData()}>Next</Button>
                        </DialogActions>
                    </Dialog>
                    <Dialog
                            ...
                            <FormControl component="fieldset">
                                <RadioGroup row aria-label="gender" name="row-radio-buttons-group">
                                    <FormControlLabel value="zip"
                                                      control={<Radio checked={this.state.exportAsZipRadioChecked}
                                                                      onChange={(e) => {
                                                                          let data = this.state;
                                    ...
                                    <FormControlLabel value="multiple"
                                                      control={<Radio checked={this.state.exportAsMultipleRadioChecked}
                                                                      onChange={(e) => {
                                                                          let data = this.state;
                ...
                <Portal>
                    <Snackbar onClose={() => this.showSuccessSnackbar(undefined)}
                              ...
                              autoHideDuration={3000}>
                        <Alert onClose={() => this.showSuccessSnackbar(undefined)} severity="success" variant={"filled"}
                               sx={{width: '100%'}}>
                ...
                <Portal>
                    <Snackbar onClose={() => this.showErrorSnackbar(undefined)}
                              ...
                              autoHideDuration={3000}>
                        <Alert onClose={() => this.showErrorSnackbar(undefined)} severity="error" variant={"filled"}
                               sx={{width: '100%'}}>
    ...
    loadClassesData(opt_back, opt_filter) {
        ...
        let data = {
            min: (this.lastFetchedClassesNum - 50) + "",
            max: this.lastFetchedClassesNum + ""
        };
        if (opt_filter) {
    ...
    renderClassesData() {
        console.log(this.state.classesData);
        if (this.state.classesData) {
            return this.state.classesData.map((item) =>
                (
                    <Grid item xs={6}>
                        <Card sx={{minWidth: 275}} variant={"outlined"} id={JSON.stringify(item)}>
                            <CardActionArea>
                                <CardContent>
                                    <div style={{
                                        display: 'flex',
                                        ...
                                        <Avatar sx={{
                                            ...
                                            fontSize: '35px',
                                            marginRight: '10px'
                                        }}>{item.type === 'class' ? "C" : (item.type === 'interface' ? "I" : "E")}</Avatar>
                                ...
                                <CardActions>
                                    ...
                                    }} startIcon={<Upload/>} size="small">Import</Button>
                                </CardActions>
                            </CardActionArea>
                        </Card>
                    </Grid>
            ...
            return (<Grid item xs={6}><CircularProgress disableShrink/></Grid>)
        }
    }
