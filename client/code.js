...
Code.init = function () {
  ...
  projectOptionsMenuItem.addEventListener("click", function () {
    ...
    editor.setValue(currentProject['androidManifest']);
    ...
    saveButtonElem.addEventListener("click", function () {
      ...
      if (autoIncrementOnPublishCheckbox.value != currentProject['incrementOnPublish']) {
        keys.push('incrementOnPublish');
        values.push(autoIncrementOnPublishCheckbox.checked ? "true" : "false");
      }
      if (proguardCheckbox.value != currentProject['proguard']) {
        keys.push('proguard');
